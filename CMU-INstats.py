...
with input from Cosma Rohilla Shalizi
...
#import BinningINSpectraAnalysis
import pandas as pd
import numpy as np
import sys
import os
...
class Freeze(object):
    ...
    def save(self, saveas=False, specType = 'ns'):
        if self.bkgd != None:
            self.data.append(self.bkgd.path + '//' + self.bkgd.name + '.csv')
        ...
        if self.path == None:
            saveas = True
        if saveas:
            self.path= input('Enter the path to save to: ') + '//' + self.name + '.csv'

        if self.interpStats is not None:
            cutpath = self.path[:self.path.rfind('/')]
            if not os.path.isdir(r'%s\spectra' % cutpath):
                os.mkdir(r'%s\spectra ' % cutpath)
            ...
            if specType == 'ns':
                keys = ['ns','nslowerCI','nsupperCI', 'diffns','diffnslowerCI','diffnsupperCI']
                for key in keys:
                    spectra[key] = np.empty(len(xs))
    
                spectra['ns'] = self.interpSpectra['ns'](xs)
                spectra['nslowerCI'] = self.interpStats['lowerCI']['ns'](xs)
                spectra['nsupperCI'] = self.interpStats['upperCI']['ns'](xs)
                spectra['diffns'] = self.interpSpectra['diffns'](xs)
                spectra['diffnslowerCI'] = self.interpStats['lowerCI']['diffns'](xs)
                spectra['diffnsupperCI'] = self.interpStats['upperCI']['diffns'](xs)
                spectra = pd.DataFrame.from_dict(spectra)
            elif specType == 'nm':
                keys = ['nm','nmlowerCI','nmupperCI', 'diffnm','diffnmlowerCI','diffnmupperCI']
                for key in keys:
                    spectra[key] = np.empty(len(xs))
    
                spectra['nm'] = self.interpSpectra['nm'](xs)
                spectra['nmlowerCI'] = self.interpStats['lowerCI']['nm'](xs)
                spectra['nmupperCI'] = self.interpStats['upperCI']['nm'](xs)
                spectra['diffnm'] = self.interpSpectra['diffnm'](xs)
                spectra['diffnmlowerCI'] = self.interpStats['lowerCI']['diffnm'](xs)
                spectra['diffnmupperCI'] = self.interpStats['upperCI']['diffnm'](xs)
                spectra = pd.DataFrame.from_dict(spectra)



            elif specType == 'K':
                keys = ['K','KlowerCI','KupperCI', 'diffK','diffKlowerCI','diffKupperCI']
                for key in keys:
                    spectra[key] = np.empty(len(xs))
    
                spectra['K'] = self.interpSpectra['K'](xs)
                spectra['KlowerCI'] = self.interpStats['lowerCI']['K'](xs)
                spectra['KupperCI'] = self.interpStats['upperCI']['K'](xs)
                spectra['diffK'] = self.interpSpectra['diffK'](xs)
                spectra['diffKlowerCI'] = self.interpStats['lowerCI']['diffK'](xs)
                spectra['diffKupperCI'] = self.interpStats['upperCI']['diffK'](xs)
                spectra = pd.DataFrame.from_dict(spectra)



            spectra.to_csv(r'%s\spectra\%s.csv' %(cutpath, self.name), index=False)
...
def initFreeze(path = None, freezes = None):
    ...
    #Problems with pixel conversion to millimeters, so we're making some assumptions here:

    if path.find('Old Pipet') != -1 or path.find('old pipet') != -1 or path.find('oldpipet') != -1:
        dV = 0.00015
    elif path.find('New Pipet') != -1:
        dV = 0.0001
    elif path.find('Microfluidic') != -1 or path.find('microfluidic') != -1:
        dV = 6E-6
    else:
        dV = 0.0001
        #Variable based on pixels per milliliter
        #Note, this is currently empirical.
        #pixPerMM = 100

        #Droplet volume in milliliters [pix]/[pix/mm] *[1cm/10mm] = mL
        #dV = (4/3)*np.pi*((freeze['Radius'][2]/pixPerMM * (1/10))**3)
...
def getColor(freeze):
    ...
    if freeze.name == 'FUE_200207_PW_AD5_1_1Difference':
        return '#332288'
    if 'OG' in freeze.att:
        return adjust_lightness('#117733', float(freeze.name[-11])/2 + 1)
...
def makePlot(freezes, what=None, where=None, raw=False, CI=False, mean=False, scale='auto', alsoraw=False, legend=True):
    ...
    if what is None: what = input('Enter what you would like to plot as a string')
...
def bkgdSubtract(freeze, bkgd, method='Empirical', CI='tskew'):
    print('Calculating background subtracted spectrum for %s' %freeze.name)
...
def loadFreeze(p, s=False):
    ...
    if s and os.path.isdir(p[:p.rfind('/') + 1] + 'spectra'):
        temp = pd.read_csv(p[:p.rfind('/') + 1] + 'spectra/' + name + '.csv')
        ...
        out.interpStats['mean'] = interpolateWellDefined(pd.DataFrame.from_dict(statstemp['mean']))
        ...
        out.interpStats['stdev'] = interpolateWellDefined(pd.DataFrame.from_dict(statstemp['stdev']))
